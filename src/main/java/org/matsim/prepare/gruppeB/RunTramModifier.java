...
*//*    public RunTramModifier(Scenario scenario, int routeOption){
    ...
    public void buildnew(Scenario scenario, String inputNetwork, String outputNetwork, TransitSchedule tschedule, String outputSchedule, String name, int choice) {
        ...
        for(int i=0; i<(12*9);i++){
            ...
            Departure dep_WD = tsfactory.createDeparture(Id.create(m10vehicle, Departure.class), 10*3600.+(i*(size-1)*60)+5.);
            dep_WD.setVehicleId(Id.createVehicleId(m10vehicle));
            m10WD.addDeparture(dep_WD);

            Departure dep_DW = tsfactory.createDeparture(Id.create(m10vehicle, Departure.class), 10*3600.+((i+1)*(size-1)*60)+5.);
    ...
    public void extend(Scenario scenario, String outputNetwork, String outputSchedule, TransitSchedule tschedule, String name, int choice){
        /** Name of Tram Line */
        final String NAME = name;  // TO DO: Get this as parameter from RunTramModifier, but does not exist yet...
